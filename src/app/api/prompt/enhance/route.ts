...
export async function POST(request: NextRequest) {
  try {
    const { prompt, mode, instruction } = await request.json();
    ...
    if (mode === 'enhance') {
      // AI优化模式：丰富细节
      systemPrompt = `你是一个专业的AI图片生成提示词优化专家。你的任务是优化用户提供的提示词，使其更适合AI图片生成。
...
7. 输出语言与输入语言保持一致`;
      ...
      userPrompt = `请优化以下图片生成提示词，使其更加详细和专业，但不要改变原本的意思：
...
${prompt}`;
      ...
      systemPrompt = `你是一个专业的AI图片生成提示词改写专家。你的任务是根据用户的修改指令来改写提示词。
